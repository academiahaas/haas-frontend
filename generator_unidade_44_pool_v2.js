...
// String limpa usando o pooler oficial do Supabase na porta 6543
const connectionString = "postgres://postgres.jdppxfokfhqjudwfwckd:aZx1jscjMZFvc6YP@aws-0-sa-east-1.pooler.supabase.co:6543/postgres";

async function run() {
  console.log("🚀 [HAAS MOTOR - BYPASS SSL] Forçando gravação da Unidade 4.4 via Porta Pooler...");
  
  const client = new Client({ 
    connectionString,
    // Configuração explícita para aceitar o certificado do pooler do Supabase
    ssl: { 
  ...
  try {
    ...
    // Busca a lição correspondente no banco (Módulo 4, Unidade 4)
    ...
    for(let i = 1; i <= totalQuestoes; i++) {
      ...
      if(tipo === 3) {
        if(i % 3 === 0) {
          ...
          exp = "Regla de oro: Las palabras terminadas en '-agem' son siempre femininas en portugués. Evita la interferencia del español 'el viaje'.";
        } else if(i % 3 === 1) {
    ...
    console.log("✨ [SUCESSO ABSOLUTO] Unidade 4.4 gravada com desvio de certificado concluído!");

  } catch (err) {
    console.error("❌ Erro fatal ao rodar script:", err.message);
  } finally {
    await client.end();
  }
}
