...
AZUL = RGBColor(0x1E, 0x3A, 0x8A) # Fundo Sólido Principal
...
def base_slide(num, hide_logo=False):
    ...
    # Fundo Azul Sólido Profundo
    sl.background.fill.solid()
    sl.background.fill.fore_color.rgb = AZUL

    # Círculos Sobrepostos Decorativos (Esferas criando profundidade)
    # Topo Direita
    ...
    p.text = "HAAS LANGUAGE  •  OVERLAPPING SPHERES EDITION"
...
def add_circle_flashcard(sl, num_str, desc, left, top, width, height, b_c=TURQUESA):
    """Cartão de prática guiada APENAS com número circular cruzando a borda, sem a palavra 'Exercício'"""
    s = sl.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, height)
...
def add_vertical_history_right(sl, history_list, left=7.8, top=1.5, width=5.0, total_height=5.3):
    ...
    for i, h_txt in enumerate(history_list):
        ...
        add_rounded_card(sl, "", clean_t, Inches(left), Inches(y), Inches(width), Inches(card_h), 
...
tb_c = sl1.shapes.add_textbox(Inches(0.5), Inches(2.2), Inches(6.0), Inches(1.8))
tf_c = tb_c.text_frame; tf_c.word_wrap = True
p = tf_c.paragraphs[0]; p.text = "[Título da Aula / Tema Principal]"
p.font.size = Pt(40); p.font.color.rgb = BRANCO; p.font.bold = True

tb_sub = sl1.shapes.add_textbox(Inches(0.5), Inches(4.2), Inches(6.0), Inches(1.2))
tf_sub = tb_sub.text_frame; tf_sub.word_wrap = True
p_sub = tf_sub.paragraphs[0]; p_sub.text = "[Subtítulo / Descrição da Aula]"
p_sub.font.size = Pt(22); p_sub.font.color.rgb = TURQUESA_CLARO; p_sub.font.bold = True

tb_date = sl1.shapes.add_textbox(Inches(0.5), Inches(5.6), Inches(6.0), Inches(0.6))
...
# SLIDE 5: METODOLOGIA 100% CIRCULAR
sl5 = base_slide(5)
...
# SLIDES 10 E 11: PRÁTICA GUIADA SEM "EXERCÍCIO"
sl10 = base_slide(10)
...
for idx in range(8):
    ...
    if idx == 0:
        add_rounded_card(sl, "", perguntas_config[0], Inches(0.5), Inches(1.5), Inches(12.3), Inches(2.7), bg_color=BRANCO, border_color=ROXO, desc_color=ROXO, desc_size=18, align_center_v=True)
        add_rounded_card(sl, "", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(4.5), Inches(12.3), Inches(2.3), bg_color=ROXO_CLARO, border_color=ROXO, desc_color=ROXO, desc_size=15, align_center_v=True)
    else:
        add_rounded_card(sl, "", perguntas_config[idx], Inches(0.5), Inches(1.5), Inches(7.0), Inches(2.7), bg_color=BRANCO, border_color=ROXO, desc_color=ROXO, desc_size=17, align_center_v=True)
        add_rounded_card(sl, "", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(4.5), Inches(7.0), Inches(2.3), bg_color=ROXO_CLARO, border_color=ROXO, desc_color=ROXO, desc_size=14, align_center_v=True)
        add_vertical_history_right(sl, [perguntas_config[i] for i in range(idx)], left=7.8, top=1.5, width=5.0, total_height=5.3)
...
out_path = "/var/www/haas-frontend-desk-mobile-oficial/public/temp-miniaturas/template-haas-modelo-12-circles.pptx"
prs.save(out_path)
print(f"🎉 MODELO 12 CIRCULAR SALVO EM: {out_path}")
