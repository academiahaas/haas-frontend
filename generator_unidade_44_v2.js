...
// Substituído o domínio por IP direto para ignorar falhas de DNS na VPS
const connectionString = "postgres://postgres:aZx1jscjMZFvc6YP@15.228.140.231:5432/postgres";

async function run() {
  console.log("🚀 [HAAS MOTOR] Iniciando geração da Unidade 4.4 via IP direto...");
  ...
  try {
    ...
    for(let i = 1; i <= totalQuestoes; i++) {
      ...
      if(tipo === 3) {
        ...
        if(i % 2 === 0) {
          dataObj = { sentence: "A viagem de negócios foi um success absoluto.", missing_word: "A" };
          respostaCorreta = "A";
    ...
    console.log("✨ [ALTO ALINHAMENTO] Unidade 4.4 totalmente injetada via IP fixo!");
  } catch (err) {
