...
def apply_bg_gradient(sl):
    """Cria um fundo em degradê sutil e dinâmico via OXML"""
    bg = sl.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), W, H)
    spPr = bg.element.spPr
    grad_xml = """
    <a:gradFill xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
      <a:gsLst>
        <a:gs pos="0"><a:srgbClr val="E2D9FF"/></a:gs>
        <a:gs pos="100000"><a:srgbClr val="D0F2F1"/></a:gs>
      </a:gsLst>
...
def base_slide(num, hide_logo=False):
    ...
    # Aplica o Fundo em Degradê Personalizado
    ...
    # Rodapé flutuante com cores contrastantes
    tb = sl.shapes.add_textbox(Inches(0.5), Inches(7.12), Inches(8), Inches(0.35))
    p = tb.text_frame.paragraphs[0]
    p.text = "HAAS LANGUAGE  •  MODERN GRADIENT EDITION"
...
def add_solid_card(sl, tit, desc, left, top, width, height, bg_color=BRANCO, border_color=ROXO, tit_color=ROXO, desc_color=SLATE, tit_size=16, desc_size=14, align_center_v=False):
...
def img_box_bleed(sl, left, top, width, height, label="[Espaço de Imagem]", bg_color=TURQUESA, text_color=BRANCO):
...
def add_vertical_history_right(sl, history_list, left=7.8, top=1.5, width=5.0, total_height=5.3):
    ...
    for i, h_txt in enumerate(history_list):
        ...
        # Cartões Brancos para contraste absoluto com o fundo degradê
        add_solid_card(sl, "", clean_t, Inches(left), Inches(y), Inches(width), Inches(card_h), 
                       bg_color=BRANCO, border_color=TURQUESA, desc_color=SLATE, desc_size=d_sz, align_center_v=True)

def add_clean_flashcard(sl, num_str, desc, left, top, width, height, b_c=ROXO):
    s = sl.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, height)
...
# ==========================================================
# CONSTRUÇÃO DOS 22 SLIDES DO MODELO 9
# ==========================================================

# SLIDE 1: CAPA COM IMAGEM VAZANDO NO TOPO (FULL WIDTH)
sl1 = base_slide(1, hide_logo=True)
img_box_bleed(sl1, Inches(0), Inches(0), W, Inches(3.5), "[Imagem de Capa / Herói no Topo]", bg_color=ROXO)
...
add_solid_card(sl2, "Professor(a):", "[Nome do(a) Professor(a)]", Inches(0.5), Inches(1.5), Inches(12.3), Inches(1.5), bg_color=BRANCO, border_color=ROXO)
add_solid_card(sl2, "Aluno(s):", "[Nome do Aluno / Lista da Turma]", Inches(0.5), Inches(3.2), Inches(12.3), Inches(3.6), bg_color=BRANCO, border_color=TURQUESA)

# SLIDE 3: WARM-UP (IMAGEM VAZANDO NO RODAPÉ FULL WIDTH)
sl3 = base_slide(3)
add_header(sl3, "Aquecimento / Warm-up", "Foco 100% na Fala (3 Minutos)")
add_solid_card(sl3, "Pergunta Provocativa Rápida:", "[Insira aqui a pergunta provocativa de aquecimento]", Inches(0.5), Inches(1.5), Inches(7.8), Inches(2.2), bg_color=BRANCO, border_color=ROXO, tit_size=20, desc_size=16)
add_solid_card(sl3, "", "Ativa o córtex pré-frontal e reduz a inibição linguística inicial.", Inches(8.5), Inches(1.5), Inches(4.3), Inches(2.2), bg_color=BRANCO, border_color=TURQUESA, desc_color=SLATE, desc_size=14, align_center_v=True)
img_box_bleed(sl3, Inches(0), Inches(4.2), W, Inches(2.8), "[Imagem do Warm-up / Rodapé]", bg_color=TURQUESA)

# SLIDE 4: OBJETIVOS
sl4 = base_slide(4)
add_header(sl4, "Objetivos da Aula")
add_solid_card(sl4, "Metas da Aula:", "• [Objetivo 1]\n• [Objetivo 2]\n• [Objetivo 3]", Inches(0.5), Inches(1.5), Inches(7.8), Inches(5.3), bg_color=BRANCO, border_color=ROXO)
add_solid_card(sl4, "Nota de Escrita:", "Nesta aula daremos atenção à escrita para reforçar a memorização de estruturas/vocabulário e reduzir a interferência da língua materna.", Inches(8.5), Inches(1.5), Inches(4.3), Inches(5.3), bg_color=BRANCO, border_color=TURQUESA)

# SLIDE 5: METODOLOGIA (Layout Diferenciado: 1 Card Vertical Esquerda + 4 em Grid na Direita)
sl5 = base_slide(5)
add_header(sl5, "A Nossa Metodologia", "Estrutura Fixa de Aprendizado Haas Language")
add_solid_card(sl5, "1. Fala Ativa", "Conversação inicial sem julgamentos.", Inches(0.5), Inches(1.5), Inches(3.8), Inches(5.3), bg_color=BRANCO, border_color=ROXO)
...
for idx, (t, d) in enumerate(met_grid):
    ...
    add_solid_card(sl5, t, d, Inches(l), Inches(t_pos), Inches(3.8), Inches(2.5), bg_color=BRANCO, border_color=TURQUESA)

# SLIDES 6 A 8: EXPOSIÇÃO (Conteúdo Largo em Cima + Divisão Embaixo)
for s_num in [6, 7, 8]:
    sl = base_slide(s_num)
    add_header(sl, f"Explicação do Tema — Parte {s_num - 5}")
    add_solid_card(sl, "Conteúdo Principal:", f"[Explicação e exemplo central da estrutura - Bloco {s_num - 5}]", Inches(0.5), Inches(1.5), Inches(12.3), Inches(2.5), bg_color=BRANCO, border_color=ROXO, tit_size=20, desc_size=15)
    add_solid_card(sl, "Exemplos Práticos:", "[Exemplos adicionais e frases do dia a dia]", Inches(0.5), Inches(4.2), Inches(6.0), Inches(2.6), bg_color=BRANCO, border_color=TURQUESA)
    add_solid_card(sl, "Notas & Detalhes:", "[Exceções, nuances gramaticais e contexto de uso]", Inches(6.8), Inches(4.2), Inches(6.0), Inches(2.6), bg_color=BRANCO, border_color=AZUL)

# SLIDE 9: SÍNTESE + CULTURA
sl9 = base_slide(9)
add_header(sl9, "Síntese do Conteúdo & Sugestão Cultural")
add_solid_card(sl9, "Resumo do Tema:", "[Resumo sintético dos pontos chave ensinados na aula]", Inches(0.5), Inches(1.5), Inches(6.0), Inches(5.3), bg_color=BRANCO, border_color=ROXO)
add_solid_card(sl9, "Sugestão Cultural:", "🎵 [Nome da Música / Vídeo recomendado sobre o assunto].\n\nO professor pode rodar o áudio/vídeo ou deixar como recomendação extra para o aluno praticar.", Inches(6.8), Inches(1.5), Inches(6.0), Inches(5.3), bg_color=BRANCO, border_color=TURQUESA)

# SLIDES 10 E 11: PRÁTICA GUIADA
sl10 = base_slide(10)
add_header(sl10, "Prática Guiada — Exercícios 1 a 5", "Correção de Vícios")
add_clean_flashcard(sl10, "01", "[Lacunas contextualizadas com opções]", Inches(0.5), Inches(1.5), Inches(12.3), Inches(1.2), b_c=ROXO)
...
add_clean_flashcard(sl10, "04", "[Reestruturação / Transformação de frase]", Inches(0.5), Inches(4.9), Inches(6.0), Inches(1.9), b_c=ROXO)
add_clean_flashcard(sl10, "05", "[Reestruturação / Transformação de frase]", Inches(6.8), Inches(4.9), Inches(6.0), Inches(1.9), b_c=ROXO)
...
add_clean_flashcard(sl11, "07", "[Escolha da opção correta]", Inches(0.5), Inches(2.9), Inches(6.0), Inches(1.8), b_c=ROXO)
add_clean_flashcard(sl11, "08", "[Substituição por sinônimo formal]", Inches(6.8), Inches(2.9), Inches(6.0), Inches(1.8), b_c=ROXO)
...
for idx in range(8):
    ...
    if idx == 0:
        ...
        add_solid_card(sl, "", p_desc, Inches(0.5), Inches(1.5), Inches(12.3), Inches(2.7), bg_color=BRANCO, border_color=ROXO, desc_color=ROXO, desc_size=18, align_center_v=True)
        add_solid_card(sl, "", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(4.4), Inches(12.3), Inches(2.4), bg_color=ROXO_CLARO, border_color=ROXO, desc_color=ROXO, desc_size=15, align_center_v=True)
    else:
        # Coluna ESQUERDA: Pergunta Ativa e Prompts
        p_desc = perguntas_config[idx]
        add_solid_card(sl, "", p_desc, Inches(0.5), Inches(1.5), Inches(7.0), Inches(2.7), bg_color=BRANCO, border_color=ROXO, desc_color=ROXO, desc_size=17, align_center_v=True)
        add_solid_card(sl, "", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(4.4), Inches(7.0), Inches(2.4), bg_color=ROXO_CLARO, border_color=ROXO, desc_color=ROXO, desc_size=14, align_center_v=True)
        
        # Coluna DIREITA: Histórico Empilhado Verticalmente
...
add_solid_card(sl20, "Tarefa de Escrita Recomendada:", "[Instrução detalhada da tarefa: escrever um texto, frases ou diálogo]", Inches(0.5), Inches(1.5), Inches(12.3), Inches(2.8), bg_color=BRANCO, border_color=ROXO)
add_solid_card(sl20, "", "Identificamos que a escrita nesta etapa é fundamental para você fixar as estruturas aprendidas, expandir vocabulário ativo e diminuir a tradução mental automática do seu idioma nativo.", Inches(0.5), Inches(4.5), Inches(12.3), Inches(2.3), bg_color=BRANCO, border_color=TURQUESA, align_center_v=True)

# SLIDE 21: TAREFA DE CASA
sl21 = base_slide(21)
add_header(sl21, "Encerramento — Tarefa de Casa", "Ação e Prática Contínua")
add_solid_card(sl21, "Tarefa de Casa:", "• [Atividade prática 1 para realizar fora da aula]\n• [Atividade de fixação de vocabulário ou escuta]", Inches(0.5), Inches(1.5), Inches(12.3), Inches(5.2), bg_color=BRANCO, border_color=ROXO)

# SLIDE 22: ENCERRAMENTO & FEEDBACK
sl22 = base_slide(22)
add_header(sl22, "Encerramento & Feedback Rápido", "Considerações Finais")
add_solid_card(sl22, "Agradecimento:", "Obrigado pela dedicação na aula de hoje!", Inches(0.5), Inches(1.5), Inches(12.3), Inches(2.0), bg_color=BRANCO, border_color=TURQUESA, tit_size=20, desc_size=16)
add_solid_card(sl22, "Espaço para Feedback Rápido:", "[Espaço destinado para o professor coletar dúvidas rápidas e considerações do aluno]", Inches(0.5), Inches(3.7), Inches(12.3), Inches(3.0), bg_color=BRANCO, border_color=ROXO, tit_size=18, desc_size=15)

out_path = "/var/www/haas-frontend-desk-mobile-oficial/public/temp-miniaturas/template-haas-modelo-9.pptx"
prs.save(out_path)
print(f"✅ MODELO 9 (MODERN GRADIENT EDITION) GERADO EM: {out_path}")
