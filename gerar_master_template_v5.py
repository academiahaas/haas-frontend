...
def add_history_cards(sl, history_questions, top=2.25, height=1.05):
    """Gera N cards individuais organizados lado a lado no topo"""
...
# SLIDES 12 A 19: CONVERSAÇÃO (COM LAYOUT DE CARDS INDIVIDUAIS DE HISTÓRICO)
perguntas_config = [
...
for idx in range(8):
    ...
    # SLIDE 12 (Pergunta 1): Sem histórico, Card Grande Preenchido
    if idx == 0:
        ...
        add_card(sl, "Sub-prompts Visuais (Apoio ao Aluno):", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(4.9), Inches(12.3), Inches(1.9), tit_size=16, desc_size=14)
    else:
        # SLIDES 13 a 19: Histórico em N Cards Lado a Lado + Pergunta Ativa
        ...
        # Sub-prompts
        add_card(sl, "Sub-prompts Visuais (Apoio ao Aluno):", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(5.5), Inches(12.3), Inches(1.35), tit_size=15, desc_size=13)
...
prs.save(out_path)
print(f"✅ NOVO TEMPLATE HAAS LANGUAGE V5 CRIADO EM: {out_path}")
