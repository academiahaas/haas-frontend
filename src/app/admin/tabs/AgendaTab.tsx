...
import { RefreshCw, CalendarClock, Plus, XCircle, CheckCircle2, X } from 'lucide-react';
...
export function AgendaTab() {
  ...
  const carregarHorarios = async () => {
    setLoading(true);
    try {
      // Busca o dia selecionado em horário de Bogotá, convertendo para o intervalo UTC correspondente
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <p className="text-[10px] text-slate-500 shrink-0">Todos los horarios se muestran en zona horaria de Colombia (Bogotá).</p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {horarios.map((h) => (
            <div key={h.id} className="bg-[#0a1424] border border-white/10 rounded-xl p-3 flex items-center justify-between">
              <div>
