...
// Configuração estrita do Dia 4: Origem Escrita e Peso 1.0
...
export async function POST(request: Request) {
  try {
    ...
    const { id_aluno, id_agenda, imagem_caderno_base64 } = body;

    console.log('--- [MÓDULO DE ESCRITA: CADERNO HAAS] ---');
    console.log('Processando caderno do aluno:', id_aluno);

    if (!id_aluno || !id_agenda) {
      return NextResponse.json({ error: 'Parâmetros obrigatórios ausentes: id_aluno e id_agenda' }, { status: 400 });
    }

    // 1. SIMULADOR DO GEMINI VISION: Analisa a caligrafia e força a padronização de tags do catálogo Haas
    console.log('[VISION SIMULATOR] Escaneando caligrafia e convertendo em telemetria...');
    const visionAnalysis = {
      desvios: [
        ...
        {
          tag: "prepositions",
          descricao: "Uso incorreto da preposição de lugar ou regência no papel",
          evidencia_aluno: "He is married with her (regência correta: married to)"
        }
      ]
    };

    // 2. Mapeamento e Ingestão na tabela relacional unificada tabela_performance_360
    ...
    if (!supabaseInsert.ok) {
      ...
      return NextResponse.json({ error: 'Erro ao salvar telemetria de escrita no banco' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      message: "Scanner de caderno integrado à fiação centralizada com sucesso.",
      dados_ingeridos: logs.length
    });

  } catch (err: any) {
    console.error('Erro Crítico no Controlador de Caderno:', err);
