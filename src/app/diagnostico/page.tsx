...
function DiagnosticoContent() {
  ...
  const handleCadastro = async () => {
    if (!nome.trim() || !email.trim() || !senha.trim()) {
      alert(
        idioma === "espanhol" ? "Por favor, complete nombre, correo electrónico y contraseña para crear su cuenta." :
        idioma === "ingles" ? "Please fill in name, email and password to create your account." :
        "Por favor, preencha nome, e-mail e senha para criar sua conta."
      );
      return;
    }
    ...
    try {
      ...
      if (emailExistente) {
        alert(
          idioma === "espanhol" ? "¡Este correo ya está registrado en el sistema! Redirigiendo al inicio de sesión..." :
          idioma === "ingles" ? "This email is already registered in the system! Redirecting to login..." :
          "Esse e-mail já está cadastrado no sistema! Redirecionando para o login..."
        );
          ...
          {step === 1 && (
            <div className="space-y-6 text-center animate-fade-in">
              ...
              <div className="grid grid-cols-1 gap-3 pt-2">
                {[
                  { id: 'ingles', label: '🇬🇧 Inglés', desc: 'Estándar Global y Profesional' },
                  { id: 'espanhol', label: '🇪🇸 Español', desc: 'Especial para Brasileños' },
                  { id: 'portugues', label: '🇧🇷 Portugués', desc: 'Especial para Hispanohablantes' }
                ].map((item) => (
                  <button
                    key={item.id}
