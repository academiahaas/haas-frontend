...
export function TrilhaOriginal({ t, router }) {
  ...
  return (
    <div style={{ left: '20.50%', top: '14.50%', width: '39.50%', height: '60.00%' }} className="absolute bg-[#162630] border border-[#233744]/60 rounded-2xl p-5 flex flex-col shadow-md overflow-y-auto gap-4 no-scrollbar">
      ...
      <div className="border-2 border-purple-500/40/80 rounded-xl p-4 bg-gradient-to-r from-[#2c1d11] to-[#1a1310] flex flex-col gap-2 shadow-lg">
        <div className="flex justify-between items-start">
          <div>
            <span className="block text-[10px] text-purple-300 font-black uppercase">{t.modulo} 3</span>
        ...
        <button type="button" onClick={manejarRedireccionReal} className="w-full bg-gradient-to-r from-purple-600 to-cyan-500 text-black font-black text-[10px] py-2 rounded-lg uppercase tracking-wider hover:bg-orange-400 transition-colors mt-1 cursor-pointer">{t.treinarReativar}</button>
      </div>
    </div>
  );
}
