...
export default function ModalAgendaAluno({ isOpen, onClose, idioma }: Props) {
  ...
  const t = {
    PT: {
      ...
      tagRegular: "📖 CLASSE REGULAR", tagReposicion: "🔄 REPOSIÇÃO",
      successMsg: "✓ Agendado com sucesso!"
    },
    EN: {
      ...
      tagRegular: "📖 REGULAR CLASS", tagReposicion: "🔄 MAKEUP CLASS",
      successMsg: "✓ Successfully booked!"
    },
    ES: {
      ...
      tagRegular: "📖 CLASE REGULAR", tagReposicion: "🔄 CLASE DE REPOSICIÓN",
      successMsg: "✓ ¡Reservado con éxito!"
    }
  }[idioma];
  ...
  const i18nCalendar = {
    ...
    ES: { weekDays: ["D", "L", "M", "X", "J", "V", "S"], months: ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julho", "Agosto", "Septiembre", "Octubre", "Novembro", "Diciembre"] }
  }[idioma];
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`relative w-full max-w-md h-full max-h-[85vh] sm:h-[540px] bg-[#030914] border border-white/[0.06] rounded-[24px] p-4 sm:p-6 flex flex-col justify-between shadow-2xl transition-all duration-300 transform ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
          ...
          <div className="flex-1 mt-4 flex flex-col justify-start relative">
            ...
            {/* ABA LISTA */}
            {activeTab === "lista" && (
              <div className="space-y-2 overflow-y-auto pr-1 flex-1 max-h-[clamp(180px,45vh,260px)] custom-scrollbar">
                {aulas.length === 0 ? (
