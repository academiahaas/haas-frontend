...
import { Home, MapPin, Gift, BookOpen, Trophy, ChevronDown, Crown, User, X, Shield, Box, Target, Globe, Flame, Clock, Award, Star, Zap, Terminal, TrendingUp, AlertTriangle } from 'lucide-react';
...
export default function DashboardDesktop() {
  ...
  // 🔒 TRAVA DE SCROLL AUTOMÁTICA DA ARENA
  useEffect(() => {
    if (isArenaOpen) {
        ...
        <div className={`relative w-[340px] h-screen max-h-screen bg-[#040c16] border-r border-white/5 py-4 px-5 flex flex-col justify-between gap-[1.5vh] shadow-2xl transition-transform duration-300 overflow-y-hidden ${isPerfilOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex flex-col gap-2 bg-[#071324]/60 p-2.5 rounded-2xl border border-white/[0.03] w-full min-h-0">
            <div className="flex items-center gap-3.5">
              ...
              <span className="text-[9px] font-mono font-black text-amber-500 uppercase tracking-widest mt-0.5">
          ...
          <div className="flex flex-col gap-[1.5vh] w-full min-h-0 flex-grow justify-between py-[1vh]">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
              ...
              <div onClick={() => (window as any).setIsPagamentoOpen ? (window as any).setIsPagamentoOpen(true) : alert('Invoice System Loading...')} className="bg-gradient-to-br from-amber-600/20 via-amber-500/10 to-transparent border border-amber-500/40 hover:border-amber-400 py-2 px-2 rounded-xl text-center flex flex-col justify-center items-center cursor-pointer transition-all hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_12px_rgba(245,158,11,0.05)] group" title="Clique para efetuar pagamento">
                <span className="text-[12px] font-mono font-black text-[#f59e0b] block leading-none group-hover:text-amber-300">149.90</span>
                <span className="font-sans text-[7.5px] font-black tracking-tight text-amber-500/90 block uppercase mt-1 animate-pulse">⚡ {idioma === 'PT' ? 'PAGAR FATURA' : idioma === 'ES' ? 'PAGAR FACTURA' : 'PAY INVOICE'}</span>
              </div>
            </div>
          </div>
...
function QuadrinhoPagamentoInteligente({ idioma }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fadeIn">
      ...
      <div className="relative w-full max-w-md bg-[#061324] border border-amber-500/30 rounded-[28px] p-6 flex flex-col gap-4 shadow-2xl">
        <div className="flex justify-between items-center border-b border-white/5 pb-3">
          <span className="text-[#f59e0b] font-mono text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5">
            ...
            {idioma === 'PT' ? 'ÁREA DE CHECKOUT HAAS' : idioma === 'ES' ? 'PASARELA DE PAGO' : 'HAAS SECURE CHECKOUT'}
          </span>
          <button onClick={() => setIsOpen(false)} className="text-slate-500 hover:text-white"><X size={16} /></button>
        </div>
        <div className="text-center py-4 bg-slate-950/40 rounded-2xl border border-white/5">
          <span className="text-slate-400 text-xs block font-medium mb-1">{idioma === 'PT' ? 'Mensalidade em Aberto' : 'Pending Invoice'}</span>
          <span className="text-3xl font-black text-white font-mono">R$ 149,90</span>
        </div>
        <button onClick={() => alert(idioma === 'PT' ? 'Redirecionando para o Gateway Pix/Cartão seguro...' : 'Redirecting to secure gateway...')} className="w-full bg-gradient-to-r from-amber-500 to-amber-600 text-black font-black py-3.5 rounded-xl text-xs uppercase tracking-widest shadow-md hover:brightness-110 transition-all">
          {idioma === 'PT' ? 'EFETUAR PAGAMENTO SEGURO' : 'PROCEED TO PAYMENT'}
        </button>
      </div>
    </div>
  );
