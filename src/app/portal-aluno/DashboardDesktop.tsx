...
import { Home, MapPin, Gift, BookOpen, Trophy, ChevronDown, Crown, User, X, Shield, Box, Target, Globe, Flame, Clock, Award, Star, Zap, Terminal, TrendingUp, AlertTriangle, Calendar, ChevronLeft, ArrowRight, Hourglass, Users, Briefcase, Ticket, FileText } from 'lucide-react';
...
export default function DashboardDesktop({ alunoData }: any) {
  ...
  useEffect(() => {
    ...
    if (isArenaOpen) {
      ...
      // ✅ Ao fechar a Arena, recarrega módulo/unidades — garante que a tela reflita
      // avanço de módulo/nível que pode ter ocorrido durante os estudos.
      (async () => {
        try {
          const uid = alunoData?.id || alunoData?.user_id || (typeof window !== "undefined" && (localStorage.getItem("haas_user_id") || (window as any).activeUserId)) || undefined;
          const dados = await buscarProgressoAlunoCentral(uid);
          ...
          console.error("Erro ao recarregar módulo/unidades após fechar Arena:", errRecarregar);
        }
      })();
    }
  ...
  const [listaEntregas, setListaEntregas] = useState([]);
    ...
    async function carregarDadosCentralizados() {
      ...
      const dynamicUid = authUser?.id || alunoData?.id || alunoData?.user_id || (typeof window !== "undefined" && (localStorage.getItem("haas_user_id") || localStorage.getItem("haas_uid") || localStorage.getItem("supabase_uid"))) || undefined;
      const dadosPortal = await fetchCentralPortalData(dynamicUid);
        ...
        if (dadosPortal && dadosPortal.user) {
          ...
          const catCentral = dbUser.plan_category || dbUser.student_type || "Free Trial";
        setTipoAluno(catCentral.toLowerCase());
          if (dbUser.clinical_precision !== undefined) setPrecisaoClinica(dbUser.clinical_precision);
          if (dbUser.total_immersion_es !== undefined && dbUser.total_immersion_es !== null) {
            const totalMinutos = Number(dbUser.total_immersion_es) || 0;
            const horas = Math.floor(totalMinutos / 60);
            const minutos = totalMinutos % 60;
            setImersaoTotal(horas > 0 ? `${horas}h ${minutos}m` : `${minutos}m`);
          }
          ...
          setCGramatica(Number(valGramatica));
          setCEscritura(Number(valEscrita));
          setCLectura(Number(valLeitura));
          ...
          if (dbUser.trained_days && Array.isArray(dbUser.trained_days)) {
            // índice 0 = Domingo (padrão Postgres EXTRACT(DOW)), por isso o mapeamento abaixo
            setTrainSun(!!dbUser.trained_days[0]);
            setTrainMon(!!dbUser.trained_days[1]);
            setTrainTue(!!dbUser.trained_days[2]);
            setTrainWed(!!dbUser.trained_days[3]);
            setTrainThu(!!dbUser.trained_days[4]);
            setTrainFri(!!dbUser.trained_days[5]);
            setTrainSat(!!dbUser.trained_days[6]);
          }
        ...
        <div className={`relative w-[340px] h-screen max-h-screen bg-[#060912] border-r border-white/10 py-5 px-4 flex flex-col gap-3  shadow-2xl transition-transform duration-300 overflow-hidden ${isPerfilOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex flex-col gap-2.5 bg-[#071324]/60 p-3.5 rounded-2xl border border-white/[0.04] w-full shrink-0">
            <div className="flex items-center gap-3.5">
              <div className="relative w-10 h-10 rounded-full bg-slate-900 border-2 border-cyan-500/40 flex items-center justify-center font-mono font-black text-cyan-400 text-base select-none">{getLastNameInitial(userName || aluno1)}<div className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-emerald-500 border-2 border-[#040c16]"></div></div>
              <div className="flex flex-col">
              <span className="text-white font-black text-sm tracking-wide">{userName || aluno1}</span>
            ...
            <div className="flex flex-col gap-1.5 border-t border-white/[0.04] pt-2.5 relative">
              ...
              <div className="grid grid-cols-2 gap-1 text-center items-stretch w-full">
              ...
              <XpCardButton totalXp={totalXp} onClick={handleAbrirArenaComTransicao} idioma={idioma} />
                ...
                <div onClick={() => setIsProvaOpen(true)} className={`relative border py-1.5 px-1 rounded-lg text-[9px] font-mono font-bold cursor-pointer transition-all active:scale-95 flex items-center justify-center ${
                  (alunoData?.exame_disponivel)
                    ? "bg-violet-500/20 border-violet-400 text-violet-200 shadow-[0_0_16px_rgba(167,139,250,0.5)] animate-pulse"
                    : "bg-slate-900/80 border-violet-500/30 hover:border-violet-400/60 hover:bg-violet-500/10 text-violet-300"
                }`}>
                  <FileText size={11} className="text-violet-400 inline-block mr-1" /> {idioma === 'PT' ? 'Provas' : idioma === 'ES' ? 'Pruebas' : 'Exams'}
                  {(alunoData?.exame_disponivel) && (
                    <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-violet-400 animate-ping" />
                  )}
                </div>
              </div>
            </div>
