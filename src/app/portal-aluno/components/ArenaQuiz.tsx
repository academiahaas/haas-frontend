    ...
    try {
      ...
      await fetch(`${supabaseUrl}/rest/v1/user_unit_progress?on_conflict=user_id,unit_id`, {
        ...
        body: JSON.stringify({
          ...
          activity_type: (activityType || jogoSelecionado) === 'paragrafos' ? '8' : ((activityType || jogoSelecionado) === 'shadowing' ? '10' : ((activityType || jogoSelecionado) === 'spelling' ? '11' : ((activityType || jogoSelecionado) === 'traducao' ? '12' : ((activityType || jogoSelecionado) === 'velocidade' ? '13' : ((activityType || jogoSelecionado) === 'escolha' ? '1' : ((activityType || jogoSelecionado) === 'caca_erro' ? '2' : ((activityType || jogoSelecionado) === 'blitz' ? '3' : ((activityType || jogoSelecionado) === 'ditado' ? '4' : ((activityType || jogoSelecionado) === 'blocos' ? '5' : ((activityType || jogoSelecionado) === 'ordenacao' ? '7' : ((activityType || jogoSelecionado) === 'leitura_veloz' || (activityType || jogoSelecionado) === 'leitura' ? '6' : (activityType || jogoSelecionado || 'geral')))))))))))),
          exercise_id: (activityType || jogoSelecionado) === 'shadowing' || (activityType || jogoSelecionado) === 'spelling' || (activityType || jogoSelecionado) == 'traducao' || (activityType || jogoSelecionado) == 'velocidade' || (activityType || jogoSelecionado) == 'escolha' || (activityType || jogoSelecionado) == 'caca_erro' || (activityType || jogoSelecionado) == 'blitz' || (activityType || jogoSelecionado) == 'ditado' || (activityType || jogoSelecionado) == 'blocos' || (activityType || jogoSelecionado) == 'leitura_veloz' || (activityType || jogoSelecionado) == 'leitura' || (activityType || jogoSelecionado) == 'ordenacao' ? String(dynamicExerciseId || typeof subUnidadeIndex === 'string' ? subUnidadeIndex : '09adf4ff-71ed-4b2b-982e-07c22fcd2cf0') : (dynamicExerciseId ? String(dynamicExerciseId) : String(jogoSelecionado || '8')),
          score: scoreObtido ?? 0,
