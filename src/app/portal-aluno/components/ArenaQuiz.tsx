...
import { buscarProgressoAlunoCentral, fetchNextExerciseForUser, clearPendingUnitCentral, clearPendingModuleCentral, clearPendingExamCentral } from "../../../services/centralService";
...
export default function ArenaQuiz({ isOpen, onClose, userId, idiomaAtivo, onAbrirPedagogo, subUnidadeTipo, subUnidadeIndex, unidadeId }: ArenaProps & { onAbrirPedagogo?: (tipo: "TEXTO" | "VIDEO") => void }) {
  ...
  useEffect(() => {
    async function carregarExercicioEscolha() {
      ...
      console.log("🎯 [ARENA -> MÚLTIPLA ESCOLHA] Disparando busca para UUID:", uId);
      if (uId) {
        const res = await getExerciseByActivityType(String(uId), 1);
        console.log("📦 [ARENA -> MÚLTIPLA ESCOLHA] Retorno da Central:", res);
        if (res.success && res.data && res.data.length > 0) {
          setExerciseEscolha(res.data[0]);
        } else {
          console.warn("⚠️ [ARENA -> MÚLTIPLA ESCOLHA] Nenhum exercicio retornado para activityType 1!");
        }
      }
    }
    carregarExercicioEscolha();
  }, [unitIdCentral, unidadeId, subUnidadeIndex]);
  ...
  const handleValidationResult = async (isCorrect: boolean, feedbackTexto?: string, pontosCustom?: number, exerciseId?: string) => {
    ...
    if (xpGanho > 0) {
      ...
      await sincronizarXpUnidadeComBanco(xpUnidade + xpGanho, jogoSelecionado === "roleplay" ? "9" : jogoSelecionado, xpGanho, exerciseId ? exerciseId : ((typeof window !== "undefined" && (window as any).__dadosBanco?.current_unit_id) || unidadeId || ""));
  ...
  const todosOsJogos = [
    ...
    { id: 'blitz', label: 'DESAFIO BLITZ', title: 'RECONHECIMENTO RÁPIDO', component: <MioloBlitzChallenge initialExerciseData={adaptiveExerciseData} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> },
    { id: 'ditado', label: 'PALAVRA OCULTA', title: 'FIXAÇÃO ACÚSTICA', component: <DitadoLacunas initialExerciseData={adaptiveExerciseData} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> },
    { id: 'blocos', label: 'BLOCOS DE GRAMÁTICA', title: 'CONSTRUÇÃO ESTRUTURAL', component: <MioloBlocos initialExerciseData={adaptiveExerciseData}  status={gameStatus} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> },
    { id: 'leitura', label: 'LEITURA VELOZ', title: 'SPRINT FLUIDEZ', component: <MioloLeituraRapida initialExerciseData={adaptiveExerciseData} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> },
    { id: 'ordenacao', label: 'ORDENAÇÃO DE FRASES', title: 'SINTAXE DE ALTO PADRÃO', component: <MioloOrdenacao initialExerciseData={adaptiveExerciseData} status={gameStatus} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> },
    { id: 'paragrafos', label: 'REORDENAÇÃO DE PARÁGRAFOS', title: 'COESÃO TEXTUAL AVANÇADA', component: <MioloReordenacaoParagrafos initialExerciseData={adaptiveExerciseData} status={gameStatus} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ''} nivelAtivo={levelCentral || ''} /> },
    ...
    { id: 'velocidade', label: 'MARCHAS DE ÁUDIO', title: 'SPRINT DE COMPREENSÃO', component: <MioloVelocidadeProgressiva initialExerciseData={adaptiveExerciseData} onValidateResult={handleValidationResult} unidadeAtiva={unitIdCentral || unidadeId || ""} nivelAtivo={levelCentral || ""} /> }
  ];
        ...
        <div className="w-full md:w-[68%] h-auto min-h-0 md:h-[82vh] md:min-h-[580px] md:max-h-[850px] bg-[#060A12] border border-white/10 border border-white/[0.04] rounded-[24px] p-5 flex flex-col justify-between backdrop-blur-md relative overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
          <div className="w-full flex justify-between items-center select-none pb-3 border-b border-white/[0.03]">
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 relative shrink-0 shadow-md border z-10 ${
              adaptiveExerciseData?.is_reviewing_exercise ? "bg-violet-500/20 text-violet-400 border-violet-500/40 scale-105" :
              ...
              <Bot size={20} />
              {adaptiveExerciseData?.is_reviewing_exercise ? (
                <span className="absolute -top-1 -right-1 bg-violet-500 text-[7px] text-white font-mono font-black px-1 rounded-sm">
                  {currentLang === "EN" ? "REVIEW" : currentLang === "ES" ? "REPASO" : "REVISÃO"}
                </span>
              ) : streak >= 3 && (
            ...
            {false ? (
              <div className="w-full h-full flex flex-col items-center justify-center text-center p-4 gap-4 select-none">
                {false ? (
                  ...
                  >
                    <Gift size={50} className="text-[#a855f7] group-hover:text-[#22C55E] transition-colors duration-300" />
              ...
              <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center text-center p-6 bg-[#070d19]/60 backdrop-blur-md rounded-2xl border border-white/[0.02] relative overflow-hidden select-none animate-[fadeIn_0.5s_ease-out]">
                {false ? (
                  <div className="flex flex-col items-center justify-center gap-2 animate-[ping_1s_infinite]">
                    <span className="text-6xl font-black font-mono text-[#a855f7]">
            ...
            <button 
              ...
              disabled={visualizacaoAtiva !== "EXERCICIO"} className={`ml-auto w-11 h-11 flex items-center justify-center text-sm font-black rounded-xl shrink-0 disabled:opacity-20 disabled:cursor-not-allowed transition-all border-none disabled:opacity-20 disabled:cursor-not-allowed disabled:pointer-events-none ${(desafioIniciado && gameStatus !== "CORRECT") || (jogoSelecionado === "blitz" && gameStatus === "CORRECT") ? "hover:-translate-y-0.5 cursor-pointer" : ""} ${
                gameStatus === 'CORRECT' && jogoSelecionado !== 'blitz'
                  ...
                  : 'bg-gradient-to-r from-[#a855f7] to-[#00e5ff] text-white pointer-events-auto opacity-100'
              }`}
            >
              ➔
            </button>
...
<span className="block w-full">
  {msg.tipo !== "user" && (msg as any).isAudioMode ? (
    <div className="flex items-center gap-3 bg-[#0B0E17] hover:bg-[#0E1526] rounded-xl px-4 py-2.5 border border-purple-500/30 hover:border-purple-500/80 transition-all duration-200 cursor-pointer shadow-[0_0_12px_rgba(168,85,247,0.12)] hover:shadow-[0_0_18px_rgba(168,85,247,0.35)] group max-w-max my-1">
      ...
      <span className="text-slate-100 text-[14px] leading-relaxed whitespace-pre-wrap break-words">{msg.texto.replace(/QUEUE:\d+/g, "").trim()}</span>
    </div>
  ) : (
    <>
      {msg.tipo !== "user" && (
      ...
      {msg.texto}
                      ...
                      {respostaIA && !chatHistory.some(m => m.texto === respostaIA) && (
                         <div className="text-left text-slate-100 block w-full">
                           <span className="block w-full">{respostaIA}</span>
                         </div>
                      )}
    ...
    <>
      ...
      <ModalConclusao
        ...
        onContinuar={() => {
          const targetUid = userId || activeUserId;
          if (targetUid) {
            if (modalConclusaoState.tipo === "UNIDADE") clearPendingUnitCentral(targetUid);
            else if (modalConclusaoState.tipo === "MODULO") clearPendingModuleCentral(targetUid);
            else if (modalConclusaoState.tipo === "NIVEL") clearPendingExamCentral(targetUid);
          }
