...
export default function MioloBlocos({
  ...
  const executarValidacaoInterna = async () => {
    ...
    try {
      ...
      if (onValidateResult) {
          ...
          const textoMentora1 = aprovado ? (incentivoCorretoBanco || feedbackTraducoes.blocos.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackTraducoes.blocos.incentivoIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno)));
      ...
      setFeedbackIA(acertou ? (feedbackCorretoBanco || feedbackTraducoes.blocos.feedbackCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (feedbackIncorretoBanco || feedbackTraducoes.blocos.feedbackIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno))));
      if (onValidateResult) {
          const blocosEsperados = (fraseOriginalGabarito || gabaritoFrase).trim().split(/\s+/).filter(Boolean);
          const blocosAluno = blocosMontados.map(b => b.texto.trim()).filter(Boolean);
          let acertos = 0;
          ...
          const aprovado = nota >= 6;
          const textoMentora2 = aprovado ? (incentivoCorretoBanco || feedbackTraducoes.blocos.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackTraducoes.blocos.incentivoIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno)));
  ...
  return (
    <div className="w-full h-full flex flex-col font-sans flex-1 min-h-0 gap-5 p-2 overflow-hidden select-none">
      ...
      <div className={`flex flex-wrap gap-3 w-full p-5 bg-[#080C16]/80 border border-slate-800/80 rounded-xl justify-center items-center shrink-0 min-h-[100px] overflow-y-auto shadow-sm ${exibirContainerInferior ? "hidden" : ""}`}>
          ...
          {localStatus === 'CORRECT' && feedbackIA && (
            <div className="w-full max-w-2xl bg-[#080C16]/90 border border-emerald-500/30 rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center text-center animate-fade-in shadow-[0_0_30px_rgba(16,185,129,0.12)] gap-3">
              <div className="flex items-center gap-2 text-emerald-400 text-[13px] font-bold uppercase tracking-widest">
                <CheckCircle size={16} /> <span>{feedbackTraducoes.titulos.blocosCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))}</span>
              </div>
          ...
          {localStatus === 'WRONG' && feedbackIA && (
            <div className="w-full max-w-2xl bg-[#080C16]/90 border border-rose-500/30 rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center text-center animate-fade-in shadow-[0_0_30px_rgba(244,63,94,0.12)] gap-3">
              <div className="flex items-center gap-2 text-rose-400 text-[13px] font-bold uppercase tracking-widest">
                <XCircle size={16} /> <span>{feedbackTraducoes.titulos.blocosIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno))}</span>
              </div>
              <p className="text-[16px] text-slate-100 font-medium italic break-words leading-relaxed">"{feedbackIA}"</p>
            </div>
          )}
