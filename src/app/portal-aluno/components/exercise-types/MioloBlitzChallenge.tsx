...
export default function MioloBlitzChallenge({
  ...
  useEffect(() => {
    if (gameOver && onValidateResult && !validadoRef.current) {
      ...
      onValidateResult(totalXp > 0, (idiomaNativoAluno?.toLowerCase().includes("por") ? `Desafio Concluído! Você conquistou ${totalXp} XP no Blitz.` : idiomaNativoAluno?.toLowerCase().includes("ing") ? `Challenge Completed! You earned ${totalXp} XP in Blitz.` : `¡Desafío Concluido! Ganaste ${totalXp} XP en Blitz.`), totalXp, exerciseId || unidadeAtiva);
    }
        ...
        <div className='w-full max-w-xl bg-[#080C16]/90 border border-[#a855f7]/30 rounded-2xl p-8 md:p-10 flex flex-col items-center justify-center text-center shadow-[0_0_35px_rgba(168,85,247,0.15)] gap-4'>
          ...
          <div className='flex flex-col gap-1'>
            <h3 className='text-[clamp(20px,2.2vw,26px)] font-black text-slate-100 tracking-wider uppercase'>
              {feedbackTraducoes.telas.desafioConcluido(obterLangKeyCompartilhado(idiomaNativoAluno))}
            </h3>
            <p className='text-[clamp(13px,1.4vw,15px)] text-slate-400 font-medium'>
              {feedbackTraducoes.telas.performanceCalculada(obterLangKeyCompartilhado(idiomaNativoAluno))}
            </p>
          </div>
  ...
  if (!jogoIniciado) {
    return (
      ...
      >
        ...
        <h3 className="text-sm font-bold text-slate-200 tracking-wider uppercase mb-1">{idiomaNativoAluno?.toLowerCase().includes("por") ? "Desafio Blitz" : idiomaNativoAluno?.toLowerCase().includes("ing") ? "Blitz Challenge" : "Desafío Blitz"}</h3>
        <p className="text-[11px] font-mono text-cyan-400 font-semibold tracking-widest animate-pulse">
          {feedbackTraducoes.telas.toqueParaIniciar(obterLangKeyCompartilhado(idiomaNativoAluno))}
        </p>
      </div>
    );
  }
    ...
    <div className='flex flex-col items-stretch justify-between w-full h-full font-sans animate-fade-in flex-1 min-h-0 gap-4 p-1 select-none'>
      ...
      <div className='flex items-center justify-between bg-[#080C16]/80 border border-slate-800/80 px-4 py-3 rounded-xl shadow-sm shrink-0'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-[11px] md:text-xs font-bold text-slate-400 tracking-widest uppercase'>{idiomaNativoAluno?.toLowerCase().includes("por") ? "Desafio Blitz" : idiomaNativoAluno?.toLowerCase().includes("ing") ? "Blitz Challenge" : "Desafío Blitz"}</span>
        </div>
