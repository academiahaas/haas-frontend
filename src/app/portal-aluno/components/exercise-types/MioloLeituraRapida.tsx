  ...
  const executarValidacaoInterna = async () => {
    ...
    try {
      ...
      if (onValidateResult) {
          ...
          const textoMentora1 = aprovado ? (incentivoCorretoBanco || feedbackCorretoBanco || feedbackTraducoes.leituraRapida.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackIncorretoBanco || feedbackTraducoes.leituraRapida.incentivoIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno)));
      ...
      setFeedbackIA(possuiMinimo ? (feedbackCorretoBanco || feedbackTraducoes.leituraRapida.feedbackCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (feedbackIncorretoBanco || feedbackTraducoes.leituraRapida.feedbackIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno))));
      if (onValidateResult) {
          const palavrasGabarito = textoGabarito.trim().split(/\s+/).filter(Boolean);
          const palavrasAluno = inputValue.trim().split(/\s+/).filter(Boolean);
          let acertos = 0;
          ...
          const aprovado = nota >= 6;
          const textoMentora2 = aprovado ? (incentivoCorretoBanco || feedbackTraducoes.leituraRapida.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackTraducoes.leituraRapida.incentivoIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno)));
          console.log("🔍 [DEBUG] onValidateResult chamado com exerciseId:", exerciseId || "UNDEFINED");
    ...
    <div className="w-full h-full flex flex-col font-sans select-none gap-4 p-2 overflow-hidden flex-1 min-h-0">
        ...
        {!modoFeedback && (
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border font-bold text-[12px] tracking-wider shrink-0 transition-all ${
            timeLeft <= 10 
              ...
              : 'text-purple-300 border-cyan-500/40/30 bg-purple-950/30'
          }`}>
            <Timer size={14} />
      ...
      <div className="bg-[#080C16]/80 border border-slate-700/50 rounded-xl flex-1 min-h-0 w-full overflow-hidden flex flex-col items-center justify-center shadow-sm p-4 md:p-6">
        {analisando ? (
          ...
          <div className="w-full h-full p-2 text-[clamp(16px,2vw,20px)] font-medium text-slate-100 leading-relaxed select-none flex-1 flex items-center justify-center overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
