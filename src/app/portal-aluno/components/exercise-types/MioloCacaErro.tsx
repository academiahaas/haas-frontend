...
export default function MioloCacaErro({ initialExerciseData, onComplete, onSelectionChange, onValidateResult, status, unidadeAtiva }: MioloProps) {
  ...
  const executarValidacaoInterna = async () => {
    ...
    try {
      ...
      const textoMentora1 = resultado.acertou ? (incentivoCorretoBanco || feedbackTraducoes.cacaErro.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackTraducoes.cacaErro.incentivoIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno)));
      ...
      setFeedbackIA(acertou ? (feedbackCorretoBanco || feedbackTraducoes.cacaErro.feedbackCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (feedbackIncorretoBanco || feedbackTraducoes.cacaErro.feedbackIncorreto(obterLangKeyCompartilhado(idiomaNativoAluno))));
      const textoMentora2 = acertou ? (incentivoCorretoBanco || feedbackTraducoes.cacaErro.incentivoCorreto(obterLangKeyCompartilhado(idiomaNativoAluno))) : (incentivoIncorretoBanco || feedbackTraducoes.cacaErro.incentivoIncorretoQuase(obterLangKeyCompartilhado(idiomaNativoAluno)));
