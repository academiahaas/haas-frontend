...
import { ListaUnidadesMobile } from "./ListaUnidadesMobile";
...
const titulosJogos: Record<string, { label: Record<string, string>; title: Record<string, string> }> = {
  ...
  ditado: { 
    label: { PT: "PALAVRA OCULTA", ES: "DICTADO PRÁCTICO", EN: "PRACTICE DICTATION" }, 
  ...
  roleplay: {
    label: { PT: "PRÁTICA DE CONVERSAÇÃO", ES: "PRÁCTICA DE HABLA", EN: "SPEAKING PRACTICE" },
    title: { PT: "ROLEPLAY COGNITIVO", ES: "ROLEPLAY COGNITIVO", EN: "COGNITIVE ROLEPLAY" }
  },
  shadowing: {
    label: { PT: "TREINO DE FALA", ES: "ENTRENAMIENTO DE HABLA", EN: "SPEAKING TRAINING" },
    title: { PT: "TREINO DE FALA", ES: "ENTRENAMIENTO DE HABLA", EN: "SPEAKING TRAINING" }
  },
  spelling: {
...
export default function PortalMobile({
 alunoData: alunoDataProp, moduloActual, onIniciarQuiz, idioma: idiomaInicial, t }: any) {
        ...
        if (dadosPortal) {
          if (dadosPortal.user) {
            if (dadosPortal.user.clinical_precision !== undefined) {
              setPrecisaoSupabase(Number(dadosPortal.user.clinical_precision));
            }
            const nomeReal = dadosPortal.user.name || dadosPortal.user.full_name || dadosPortal.user.first_name || dadosPortal.user.nome;
            if (nomeReal && typeof setNomeUsuarioDb === "function") {
              setNomeUsuarioDb(nomeReal);
            }
            if (dadosPortal.user.current_level && typeof setNivelAluno === "function") {
              setNivelAluno(dadosPortal.user.current_level);
            }
          }
          if (dadosPortal.subscription) {
            ...
            if (sub.class_credits_available !== undefined && sub.class_credits_available !== null) {
              setCreditosRegulares(Number(sub.class_credits_available));
            }
            if (sub.replacement_credits !== undefined && sub.replacement_credits !== null) {
              setCreditosReposicao(Number(sub.replacement_credits));
            }
            if (sub.expiration_date) {
              setExpirationDate(sub.expiration_date);
            }
            if (sub.plan_category) {
              setPlanCategory(sub.plan_category);
            }
          }
        }
      } catch (e) {
        console.error("Erro ao buscar dados no Mobile:", e);
      }
    }
    carregarDadosMobile();
  }, []);
  ...
  const salvarCreditosNoSupabase = async (novosRegulares: number, novosReposicao: number) => {
    try {
      let targetUid: string | null | undefined = null;

      // 1. Tentar pegar da sessao atual do Supabase
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user?.id) {
        targetUid = session.user.id;
      }

      // 2. Se nao encontrar, varrer o localStorage
      if (!targetUid && typeof window !== "undefined") {
        targetUid = localStorage.getItem("haas_uid") || 
                    localStorage.getItem("user_id") || 
                    localStorage.getItem("supabase_uid");
        
        // 3. Tentar extrair do token do Supabase no localStorage se existir
        if (!targetUid) {
          const sbAuth = Object.keys(localStorage).find(k => k.includes("auth-token"));
          if (sbAuth) {
            try {
              const parsed = JSON.parse(localStorage.getItem(sbAuth) || "{}");
              targetUid = parsed?.user?.id || parsed?.currentSession?.user?.id;
            } catch (e) {}
          }
        }
      }
      ...
      if (!targetUid) {
        console.warn("⚠️ [SUPABASE SYNC] Nenhum UID encontrado apos busca exaustiva.");
        return;
      }
      ...
      const { error } = await supabase
        ...
        .update({
          class_credits_available: novosRegulares,
          replacement_credits: novosReposicao
        })
        .eq("user_id", targetUid);

      if (error) {
        console.error("❌ [SUPABASE SYNC] Erro Supabase:", error.message);
      } else {
        console.log("✅ [SUPABASE SYNC] Sucesso! UID:", targetUid, "| Regulares:", novosRegulares, "| Reposicao:", novosReposicao);
      }
    } catch (err) {
      console.error("❌ [SUPABASE SYNC] Excecao:", err);
    }
  };
  ...
  const [botPhraseIndex, setBotPhraseIndex] = React.useState(0);
  React.useEffect(() => {
    const interval = setInterval(() => {
    ...
    async function sincronizarPerfilMobile() {
      try {
        ...
        if (dadosPortal && dadosPortal.user) {
          ...
          if (dadosPortal.user.current_level) {
            setNivelAluno(dadosPortal.user.current_level);
          }
        }
      } catch (err) {
        console.error("Erro ao sincronizar perfil no Mobile:", err);
      }
    }
...
React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    if (alunoDataProp?.plan_category) setPlanCategory(alunoDataProp.plan_category);
        ...
        console.error("Erro ao carregar dados unificados do Mobile:", e);
      }
    }
    carregarNomeReal();
  }, [alunoData]);
...
{/* CONTAINER EXCLUSIVO: RENDERIZA APENAS AS NOVAS AULAS AGENDADAS LOCAIS */}
              ...
              <div className="bg-slate-900/40 border border-white/[0.02] p-3 rounded-xl flex items-center gap-1.5.5">
                ...
                <div className="flex flex-col">
                  <span className="text-lg md:text-2xl font-mono font-black text-white">{(() => {
                    const totalMinutos = Number(horasAtivas) || 0;
  ...
  return Array.isArray(arr) ? arr.filter(Boolean).length : 0;
                ...
                <div className="flex justify-between items-center gap-0.5 mt-0.5 border-t border-white/[0.03] pt-1">
                  {(idiomaSelecionado === "PT" ? ["D", "S", "T", "Q", "Q", "S", "S"] : idiomaSelecionado === "ES" ? ["D", "L", "M", "M", "J", "V", "S"] : ["S", "M", "T", "W", "T", "F", "S"]).map((dia, idx) => {
...
<button onClick={() => { setAbaAtiva('inicio'); setArenaAtiva(false); setStatusRespostaMobile('IDLE'); }} className={`flex flex-col items-center justify-center gap-0.5 flex-1 border-none bg-transparent cursor-pointer ${(abaAtiva as string) === 'inicio' ? 'text-cyan-400 font-black' : 'text-slate-500 font-bold'}`}>
      ...
      <ModalCofreErrosMobile
        isOpen={gavetaErroresAberta}
        onClose={() => setGavetaErroresAberta(false)}
        idiomaSelecionado={idiomaSelecionado as "PT" | "EN" | "ES"}
        userId={alunoData?.id || alunoData?.user_id}
      />
