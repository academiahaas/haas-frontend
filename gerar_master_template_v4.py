...
# SLIDE 2: PRESENÇA & INTEGRANTES (Design Flexível)
sl2 = base_slide(2)
...
alunos_txt = "1. [Nome do Aluno]\n2. [Nome do Aluno]\n3. [Nome do Aluno]\n4. [Nome do Aluno]\n5. [Nome do Aluno]\n6. [Nome do Aluno]\n7. [Nome do Aluno]\n8. [Nome do Aluno]"
add_card(sl2, "Alunos Presentes:", alunos_txt, Inches(4.6), Inches(2.3), Inches(8.2), Inches(4.3), tit_size=18, desc_size=14)
...
# SLIDES 12 A 19: CONVERSAÇÃO (8 PERGUNTAS DISTRIBUÍDAS)
perguntas_config = [
    ("Pergunta 1:", "[Pergunta principal 1] — Inicial / Descoberta"),
    ("Pergunta 2:", "[Pergunta principal 2] — Básico / Conexão"),
    ("Pergunta 3:", "[Pergunta principal 3] — Básico / Conexão"),
    ("Pergunta 4:", "[Pergunta principal 4] — Intermediário / Autônomo"),
    ("Pergunta 5:", "[Pergunta principal 5] — Intermediário / Autônomo"),
    ("Pergunta 6:", "[Pergunta principal 6] — Independente"),
    ("Pergunta 7:", "[Pergunta principal 7] — Independente"),
    ("Pergunta 8:", "[Pergunta principal 8] — Avançado / Especialista"),
]

for idx in range(8):
    s_num = 12 + idx
    sl = base_slide(s_num)
    add_header(sl, "Conversação", f"Pergunta {idx + 1} de 8")
    
    # Se houver histórico de perguntas anteriores, exibe no topo de forma limpa
    if idx > 0:
        hist_lines = [f"{i+1}. {perguntas_config[i][1].split(' — ')[0]}" for i in range(idx)]
        hist_txt = "\n".join(hist_lines)
        add_card(sl, "Histórico de Perguntas Anteriores:", hist_txt, Inches(0.5), Inches(2.3), Inches(12.3), Inches(1.3), bg_color=FUNDO_CARD, border_color=MUTED, tit_size=13, desc_size=12)
        top_act = 3.8
    else:
        top_act = 2.4
    ...
    add_card(sl, p_tit, p_desc, Inches(0.5), Inches(top_act), Inches(12.3), Inches(1.7), bg_color=ROXO_CLARO, border_color=ROXO, tit_size=18, desc_size=15)
    
    # Sub-prompts
    top_prompt = top_act + 1.9
    add_card(sl, "Sub-prompts Visuais (Apoio ao Aluno):", "• Para expandir: 'Dê um exemplo recente...'\n• Para concordar/discordar: 'Qual é o outro lado dessa moeda?'", Inches(0.5), Inches(top_prompt), Inches(12.3), Inches(1.2), tit_size=15, desc_size=13)

# SLIDE 20: PARTE ESCRITA
sl20 = base_slide(20)
add_header(sl20, "Atividade de Escrita", "Consolidação e Memorização")
add_card(sl20, "Tarefa de Escrita Recomendada:", "[Instrução detalhada da tarefa: escrever um texto, frases ou diálogo dependendo do nível]", Inches(0.5), Inches(2.4), Inches(12.3), Inches(2.6), tit_size=18, desc_size=15)
...
prs.save(out_path)
print(f"✅ TEMPLATE MASTER V4 COM 22 SLIDES GERADO EM: {out_path}")
